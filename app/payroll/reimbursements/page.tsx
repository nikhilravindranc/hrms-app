...
import { XIcon, CheckIcon, ReceiptIcon } from '@/components/Icons'
import { mockReimbursements, Reimbursement } from '@/lib/payrollData'

type Tab = 'Pending' | 'Approved' | 'Rejected' | 'Paid'
const TAB_VALUES: Tab[] = ['Pending', 'Approved', 'Rejected', 'Paid']

export default function ReimbursementsPage() {
  return (
    <Suspense fallback={null}>
      <ReimbursementsPageInner />
    </Suspense>
  )
}

function ReimbursementsPageInner() {
  const { isDark } = useTheme()
  const searchParams = useSearchParams()
  const [tab, setTab] = useState<Tab>('Pending')
  const [claims, setClaims] = useState(mockReimbursements)
  ...
  const filtered = useMemo(() => claims.filter(c => c.status === tab), [claims, tab])
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className={`text-xl font-extrabold ${textColor}`}>Reimbursements</h1>
        <p className={`text-xs font-medium mt-0.5 ${textSecondary}`}>Review and process employee expense claims.</p>
      </div>
          ...
          {TAB_VALUES.map(t => (
            ...
            >
              ...
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${tab === t ? 'bg-white/25' : isDark ? 'bg-[#27272A]' : 'bg-[#E8EFF6]'}`}>
                {claims.filter(c => c.status === t).length}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className={`rounded-xl border ${borderColor} ${cardBg} overflow-hidden`}>
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b ${borderColor}`}>
                ...
                {['Employee', 'Category', 'Amount', 'Description', 'Submitted', ''].map(h => (
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} className={`border-b ${borderColor} last:border-b-0 transition-colors cursor-pointer ${rowHover}`} onClick={() => setOpenId(c.id)}>
                  ...
                  <td className={`px-4 py-3 text-sm font-medium whitespace-nowrap ${textColor}`}>{c.category}</td>
                  <td className={`px-4 py-3 text-sm font-bold whitespace-nowrap ${textColor}`}>₹{c.amount.toLocaleString('en-IN')}</td>
                  <td className={`px-4 py-3 text-sm font-medium max-w-xs truncate ${textSecondary}`}>{c.description}</td>
                  <td className={`px-4 py-3 text-sm font-medium whitespace-nowrap ${textSecondary}`}>{new Date(c.submittedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td>
                  <td className="px-4 py-3 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                    {c.status === 'Pending' ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => setStatus(c.id, 'Approved')} className="px-2.5 py-1 rounded-md text-xs font-semibold text-white bg-[#00755A] hover:bg-[#004A3A] transition-colors">Approve</button>
        ...
        {filtered.length === 0 && (
          <div className="py-16 text-center">
            <p className={`text-sm font-medium ${textSecondary}`}>No {tab.toLowerCase()} claims.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-end bg-black/40" onClick={() => setOpenId(null)}>
          <div className={`w-full max-w-md h-full overflow-y-auto border-l ${borderColor} ${cardBg} p-6`} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className={`text-base font-bold ${textColor}`}>Claim Detail</h2>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm font-bold ${textColor}`}>{open.employeeName}</p>
                  <p className={`text-[11px] font-medium ${textSecondary}`}>{open.category}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className={`text-[10.5px] font-semibold uppercase tracking-[0.05em] ${textSecondary}`}>Amount</p>
                  <p className={`text-sm font-bold mt-0.5 ${textColor}`}>₹{open.amount.toLocaleString('en-IN')}</p>
                </div>
                <div>
                  <p className={`text-[10.5px] font-semibold uppercase tracking-[0.05em] ${textSecondary}`}>Submitted</p>
                  <p className={`text-sm font-medium mt-0.5 ${textColor}`}>{new Date(open.submittedDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                </div>
              </div>
              <div>
                <p className={`text-[10.5px] font-semibold uppercase tracking-[0.05em] ${textSecondary}`}>Description</p>
                <p className={`text-sm font-medium mt-0.5 ${textColor}`}>{open.description}</p>
              </div>
              <div>
                <p className={`text-[10.5px] font-semibold uppercase tracking-[0.05em] ${textSecondary}`}>Status</p>
                <span className={`inline-block mt-1 px-2 py-1 rounded-full text-[11px] font-semibold ${statusColors[open.status].bg} ${statusColors[open.status].text}`}>{open.status}</span>
              </div>

              {open.status === 'Pending' && (
                <div className="flex items-center gap-2 pt-2">
                  <button onClick={() => { setStatus(open.id, 'Approved'); setOpenId(null) }} className="flex-1 px-3.5 py-2 rounded-lg text-sm font-semibold text-white bg-[#00755A] hover:bg-[#004A3A] transition-colors">Approve</button>
